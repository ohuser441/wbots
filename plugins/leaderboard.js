...
Kamu: *${usersCoin.indexOf(m.sender) + 1}* dari *${usersCoin.length}*

${sortedCoin.slice(0, len).map(({ jid, coin }, i) => `${i + 1}. ${participants.some(p => jid === p.jid) ? `(${conn.getName(jid)}) wa.me/` : '@'}${jid.split`@`[0]} *${coin} COIN*`).join`\n`}
...
`.trim()
  conn.reply(m.chat, text, m, {
    contextInfo: {
      mentionedJid: [...usersMoney.slice(0, len), ...usersExp.slice(0, len), ...usersCoin.slice(0, len), ...usersLevel.slice(0, len)].filter(v => !participants.some(p => v === p.jid))
    }
  })
}
